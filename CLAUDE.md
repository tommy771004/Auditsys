...
- **`src/db/`** — Drizzle schema (`schema.ts`, tables prefixed `audit_*`) is the single source of truth for the DB shape. Migrations live in `drizzle/` (generated via `npm run db:generate`, tracked in git); the baseline migration is hand-verified idempotent (`CREATE TABLE IF NOT EXISTS`, guarded `ADD CONSTRAINT`) so it applies cleanly to both a fresh database and the existing production one. `initDb()` runs `drizzle-orm`'s `migrate()` against `drizzle/`, then seeds default `plan_settings` (free/pro/enterprise) via `onConflictDoNothing()`. Schema changes: edit `schema.ts`, then run `npm run db:generate` to produce a new migration file — never hand-edit `drizzle/` migrations that have already shipped. `adminBootstrap.ts` (`resolveAdminBootstrapConfig`) parses/validates the `BOOTSTRAP_ADMIN_*` env (throws if password < 12 chars) and is consumed by `initDb()`.
