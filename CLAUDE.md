...
Notes:
- Tests run on the native Node test runner via `tsx`, not Vitest/Jest. Current coverage lives in `test/security-hardening.test.ts` (JWT-secret fail-closed, admin bootstrap, SSRF/private-IP guards, plan parsing, deterministic collector); add new `*.test.ts` files under `test/`.
