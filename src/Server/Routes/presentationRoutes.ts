...
import { assertSafeAuditTargetUrl, UNSAFE_AUDIT_TARGET_ERROR } from "../Services/securityPolicies";
...
import { fetchOpenRouterWithFallback, fetchAgentRouter } from "../Services/openrouterHelper";
...
async function collectPresentationEvidence(targetUrl: string): Promise<PresentationEvidence> {
  // SSRF guard FIRST — throws UNSAFE_AUDIT_TARGET_ERROR on private/reserved/credentialed targets.
  await assertSafeAuditTargetUrl(targetUrl);
  const [deterministic, crux] = await Promise.all([
    collectDeterministicEvidence({ url: targetUrl }).catch((err: unknown) => {
      if (process.env.NODE_ENV !== "production") {
        console.error("Presentation deterministic probe failed:", err instanceof Error ? err.message : err);
      }
      return null;
    }),
    fetchCruxReport(targetUrl).catch((err: unknown) => {
      if (process.env.NODE_ENV !== "production") {
        console.error("Presentation CrUX fetch failed:", err instanceof Error ? err.message : err);
      }
      return null;
    }),
  ]);
...
function generateSmartPresentationFallback(
  url: string,
  techStack: string,
  knownIssues: string,
...
presentationRouter.post("/", authenticateToken, requirePlanLimits, async (req, res) => {
  ...
  void host; // used implicitly in prompt
  ...
  // Gather real measurements before LLM call.
  ...
  if (apiKey) {
    ...
    try {
      ...
      const languageInstruction = isZh
        ...
        : `1. You MUST write the ENTIRE presentation JSON content in ${language || 'English'} language.`;
