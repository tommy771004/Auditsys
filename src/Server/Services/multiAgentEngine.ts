  ...
  async run(
    ...
    try {
      ...
      const cruxDetails = crux.hasData
        ? `Field data found (p75 metrics): LCP=${crux.metrics?.lcp?.p75 ?? "N/A"}s (${crux.metrics?.lcp?.rating}), INP=${crux.metrics?.inp?.p75 ?? "N/A"}ms (${crux.metrics?.inp?.rating}), CLS=${crux.metrics?.cls?.p75 ?? "N/A"} (${crux.metrics?.cls?.rating}), FCP=${crux.metrics?.fcp?.p75 ?? "N/A"}s (${crux.metrics?.fcp?.rating}). Collection scope=${crux.collectionPeriod ?? "none"}.`
      ...
      const parsed = await LlmPipeline.callWithRetry<AgentFinding[]>(
        ai,
        "gemini-3.5-flash",
        prompt,
        {
        ...
        2,
        (attempt, err: any) => {
          this.fireLog("running", "warn", `LLM output JSON parsing failed (Attempt ${attempt}/2): ${err.message}. Retrying with feedback...`, `LLM 輸出 JSON 解析失敗 (嘗試 ${attempt}/2): ${err.message}。正在注入反饋並重試...`);
        }
      );
