...
export default function LanguageSwitcher() {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        type="button"
        className="inline-flex min-h-[44px] items-center gap-2 rounded-sm border border-black bg-white px-4 py-2 text-sm font-bold text-black transition hover:bg-black hover:text-white focus-visible:ring-2 focus-visible:ring-black focus-visible:outline-none shadow-[2px_2px_0_rgba(0,0,0,1)] hover:shadow-none translate-y-0 hover:translate-y-[2px] hover:translate-x-[2px]"
        onClick={() => {
      ...
      >
        ...
        <ChevronDown className={["h-4 w-4 text-black transition-transform", isOpen ? "rotate-180" : ""].filter(Boolean).join(" ")} />
      </button>
      ...
      <AnimatePresence>
        {isOpen ? (
          <motion.div
            ...
            className="absolute right-0 top-[calc(100%+0.75rem)] z-50 min-w-[12rem] rounded-sm border border-black bg-white p-2 shadow-[4px_4px_0_rgba(0,0,0,1)]"
          >
            <p className="px-3 pb-2 pt-1 text-xs font-bold uppercase tracking-[0.24em] text-black/60">{t("languageSwitcher.menuLabel")}</p>
              ...
              {languageOptions.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive ? <Check className="h-4 w-4 text-black" /> : null}
                  </button>
                );
              })}
            </div>
