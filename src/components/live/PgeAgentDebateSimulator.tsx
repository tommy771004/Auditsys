    ...
    <div id="pge-debate-simulator-container" className="rounded-sm border border-blue-500/30 bg-white p-6 md:p-8 backdrop-blur-xl shadow-[0_0_50px_rgba(59,130,246,0.06)] ring-1 ring-blue-500/10 mt-8">
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4 rounded-sm border border-black/5 bg-white p-5 space-y-4">
          ...
          {/* Planner Card */}
          <div className={`p-3 rounded-sm border transition-all duration-300 ${
            activeStepIndex >= 0 && debateTimeline[activeStepIndex].role === "planner"
              ...
              : "border-black/5 bg-neutral-100/40 opacity-70"
          }`}>
          ...
          {/* Audit Metrics */}
          <div className="rounded-sm border border-black/5 bg-neutral-100/20 p-4 space-y-3 pt-4">
            ...
            <div className="pt-2">
              <div className="flex justify-between text-[11px] mb-1">
                <span className="text-black/60">{isZh ? "產出品質分數" : "Evaluator Match Score"}</span>
                <span className={`font-mono font-bold ${
                  getActiveRating() >= 90 ? "text-emerald-400" : getActiveRating() > 0 ? "text-rose-400" : "text-black/40"
                }`}>
        ...
        <div className="lg:col-span-8 flex flex-col min-h-[350px] rounded-sm border border-black bg-white overflow-hidden relative">
          ...
          >
            {displayedStory.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-8 text-black/30 space-y-2 select-none">
                <BrainCircuit className="h-10 w-10 text-black/10 animate-pulse" />
                  ...
                  >
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-black/90">{turn.title}</span>
                        <span className="text-[10px] text-black/30 font-mono">Round {turn.round}</span>
                      </div>
                      <div className={`p-3 rounded-sm border ${turn.color} mt-1`}>
                        ...
                        {turn.rating !== undefined && (
                          <div className="mt-2 pt-2 border-t border-black/5 flex items-center justify-between text-[10px] font-mono">
                            <span className="text-black/40">{isZh ? "一審評核分數" : "Audit score metric:"}</span>
